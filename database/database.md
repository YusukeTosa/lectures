...
## はじめに
普段なんとなくでDjangoのmodels.pyでデータベース設計を行っている場合、migrate時のエラーの解消に手間取ったり、システムダウンなどの理由でデータベースに直接アクセスする必要が生じ際に適切な対応がとれなくなる恐れがある。そのような場合に備え、データベース周りの基本的な知識を身につけておくことは重要である。  
今回のセミナーでは、プロジェクトの企画段階からゼロベースでDB設計し、SQLで実装できるようになることを目標とする。  
なお時間の都合上、トランザクションやACID特性など普段の開発ではあまり意識しない領域の話題や、複雑なSQL構文は今回扱わない。
...
## RDBの構造  
RDB(Relational DataBase)は、データを行と列からなる2次元の表で管理する。行は個々のデータを、列はデータを構成する要素を表す。  
...
代表的なOSSのみ紹介  
* SQLite
    アプリケーションの一部に組み込まれて動作するタイプのRDB。大規模開発には向かない。
* MySQL
    世界で最もよく使用されているオープンソースのRDB。
* PostgreSQL
    MySQLと同様、世界で最もよく利用されているオープンソースのRDB。heroku標準のDBなので、herokuでデプロイするならこれが楽。
* MariaDB
    MySQLから派生。MySQLとの互換を持ちさらに性能向上している。特に制限がなければこのRDBを用いるべきかも知れない。
